     ...
     <div class="row row-header">
            <div class="col-xs-12">
                <ol class="breadcrumb">
				     ...
				     <li class="active">Неотвеченные вопросы </li>
			    </ol>
            </div>
        </div>
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading"> <b class='tb_header'> Неотвеченные вопросы </b> </div>
                ...
                <div class="panel-body">
                    ...
                    <tbody>
                     ...
                     <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{ $i+2}}"> Ответить </a>
                         <div id="collapse{{ $i+2 }}" class="panel-collapse collapse">
                             <form action="answer/{{ $item->id }} / {{ $item->tag }}" method="post">
                                  <textarea type="text" id ="Question" name="answer" rows="8" cols="50"></textarea> </br>
                                   <input type="hidden" name="_token" value="{!! csrf_token() !!}">
                                  <button type="submit" name="submitanswer"> Сохранить ответ </button> 
                             </form>
                         </div> |
                     <a href="makeNonVisible/{{ $item->id }}"> Невидимый </a> |
                      <a href="makeVisible/{{ $item->id }}"> Видимый  </a> |
                      <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{ $i+3}}"> Категория </a>
                         <div id="collapse{{ $i+3 }}" class="panel-collapse collapse">
                               <form action="changeUnaQuestionTag/{{ $item->id}} /{{$item->tag }}"  method="POST">    
